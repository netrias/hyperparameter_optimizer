...
class DARTSOptimizer:
    ...
    def build_model(self, architecture):
        ...
        dropout_rate = architecture.get('dropout_rate', 0.5)  # Default dropout rate
        for i in range(architecture['num_layers']):
            output_dim = min(input_dim, architecture['num_nodes'][i])
            layers.append(nn.Linear(input_dim, output_dim))
            layers.append(nn.ReLU())
            if i < architecture['num_layers'] - 1:  # Typically, dropout isn't added after the last layer
    ...
    def objective(self, architecture):
        ...
        learning_rate = architecture.get('learning_rate', 0.01)  # Default learning rate
    ...
    def optimize(self, X_train, y_train, X_test, y_test):
        ...
        search_space = {
            ...
            'learning_rate': hp.loguniform('learning_rate', -4, -2)  # From 0.01 to 0.1
        }
